...
### FAIL-004: master ブランチへの直接コミット (2026-04-11)

- **事象**: skill 標準化作業で Issue もブランチも作らず master に 2 コミット直接追加。後から Issue を作成し、ブランチに移し替えて PR 化するリカバリが必要になった
- **原因**: 作業開始時に `dev-start` オーケストレーターを起動せず、そのまま編集を始めた。github-flow の「Issue → ブランチ → PR」前提が暗黙知のままだった
- **対策**: `dev-start` スキルに「ファイル編集の前にブランチが feature ブランチであることを確認する」ステップを追加。CLAUDE.md に「master への直コミット禁止」を明記
- **反映先**: `.claude/skills/dev-start/SKILL.md`, `CLAUDE.md`
