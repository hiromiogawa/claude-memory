...
/**
 * PostgreSQL 永続化リポジトリの戻り値型。
 * StorageRepository の全メソッドに加えて、ライフサイクル管理用の
 * `migrate` と `close` を公開する。
 */
...
/**
 * pgvector によるベクトル検索と pg_bigm によるキーワード検索を備えた
 * PostgreSQL 永続化リポジトリを生成する。
 * @param connectionString - PostgreSQL 接続 URL
 * @param options - コネクションプール最大数などのオプション
 */
export function definePostgresStorageRepository(
  ...
  /** 検索ヒットした記憶の lastAccessedAt を現在時刻に更新し、access_count をインクリメントする */
  const touchLastAccessed = async (ids: string[]): Promise<void> => {
  ...
  return {
    ...
    async list(listOptions: ListOptions): Promise<Memory[]> {
      const {
        limit,
        offset,
        source,
    ...
    async searchByKeyword(
      ...
      const conditions = [sql`${memories.content} LIKE ${pattern} ESCAPE '\\'`]
      ...
      await touchLastAccessed(rows.map((r) => r.id))
    ...
    async countAll(): Promise<number> {
      const result = await db.select({ count: sql<number>`COUNT(*)::int` }).from(memories)
      return result[0]?.count ?? 0
    },
