---
name: dev-complete
description: 実装完了時のオーケストレーター。セルフレビュー→ドキュメント確認→コミット→PR作成を順次実行する。superpowers:verification-before-completion と併用する。
---
...
self-review スキルに従い、全チェックをパスするまで修正する:

```
pnpm lint → pnpm test → pnpm dep-check → pnpm knip → e2e-coverage
```

e2e-coverage は判断項目: 変更がインフラ層・ユースケース層にかかっている場合、対応する E2E テストが存在するか確認し、不足していれば追加する。

全パスするまでコミットしない。

### Step 2: docs-freshness（ドキュメント確認）

docs-freshness スキルに従い:
